...
public class AuthenticationHandler extends SimpleChannelInboundHandler<C2SMessage> {
	...
	protected void messageReceived(ChannelHandlerContext ctx, C2SMessage msg)
		 ...
		 if(MessageType.C2S_CUSTOMER_AUTH.equals(msgType)) // authentification information from client
		 {
			    CustomerAuthenticationMessage bodyMessage = (CustomerAuthenticationMessage) msg;
			    CustomerRegisterParameter crp = new CustomerRegisterParameter(bodyMessage.getUsername(), bodyMessage.getPassword());
			    RegisterRequest registerRequest = new RegisterRequest((SocketChannel)ctx.channel(), crp, ClientType.SUBSCRIBER);
		 		RegisterResult registerResult = scm.registerClient(registerRequest);
			    if(registerResult!=null && registerResult.isSuccess())
		 		{    
		 			  ...
		 			  if(logger.isInfoEnabled())
		 				  logger.info("[:)] New user logined successfully! username:'{}'", bodyMessage.getUsername());
