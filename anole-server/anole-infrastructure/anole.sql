...
CREATE TABLE `anole_user_project_map` (
  ...
  `Role` tinyint(3) unsigned zerofill NOT NULL DEFAULT '000' COMMENT '角色： 0-stranger 陌生人：不用插入纪录，可以看Key，但是看不到value。 1-vistor访客：可以看key和value： 2-admin管理员：可以编辑配置。 3-owner: 可以删除配置。（注意拥有者具有所有环境的管理员权限）',
  `CreateTime` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `UpdateTime` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`Id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
